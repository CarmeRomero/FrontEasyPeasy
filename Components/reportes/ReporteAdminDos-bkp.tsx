  ...
  useEffect(() => {
    ...
    tickets
      ? tickets.map((fecha: any) => {
          ...
          console.log(dia);
          dia == "lunes"
            ? setIngresoLunes((lun += parseInt(fecha.total)))
            : dia == "martes"
            ? setIngresoMartes((mar += parseInt(fecha.total)))
            : dia == "miércoles"
            ? setIngresoMiercoles((mier += parseInt(fecha.total)))
            : dia == "jueves"
            ? setIngresoJueves((jue += parseInt(fecha.total)))
            : dia == "viernes"
            ? setIngresoViernes((vie += parseInt(fecha.total)))
            : dia == "sábado"
            ? setIngresoSabado((sab += parseInt(fecha.total)))
            : setIngresoDomingo((dom += parseInt(fecha.total)));
        })
      : [];
  }, [tickets]);
